...
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_CHECK_CMDLINE_PARAMETER(n, t, d)
  ...
  if (cxt_.compression_ != "") {
    RCLCPP_INFO(get_logger(), "Using compression: %s", cxt_.compression_.c_str());
    cimage_pub_ = create_publisher<sensor_msgs::msg::CompressedImage>("image_compressed", 10);
    if (cxt_.split_frame_) {
      cimage_pub2_ = create_publisher<sensor_msgs::msg::CompressedImage>("image_compressed2", 10);
    }
  } else {
    RCLCPP_INFO(get_logger(), "Not using compression");
...
void OpencvCamNode::loop() {
  ...
  while (rclcpp::ok() && !canceled_.load()) {
    ...
    if (cxt_.split_frame_) {
      ...
      if (cxt_.compression_ != "") {
        create_and_publish_cimage(cxt_.compression_, stamp, left, cxt_.camera_frame_id_, cimage_pub_);
        create_and_publish_cimage(cxt_.compression_, stamp, right, cxt_.camera_frame_id2_, cimage_pub2_);
      } else {
      ...
      if (cxt_.compression_ != "") {
        create_and_publish_cimage(cxt_.compression_, stamp, frame, cxt_.camera_frame_id_, cimage_pub_);
      } else {
...
void OpencvCamNode::create_and_publish_image(rclcpp::Time &stamp, cv::Mat &frame, std::string frame_id, rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr pub)
{
  auto img = create_image_msg(stamp, frame, frame_id);
  pub->publish(std::move(img));
}

void OpencvCamNode::create_and_publish_cimage(std::string compression, rclcpp::Time &stamp, cv::Mat &frame, std::string frame_id, rclcpp::Publisher<sensor_msgs::msg::CompressedImage>::SharedPtr pub)
{
  auto cimg = create_cimage_msg(stamp, frame, frame_id, compression);
  pub->publish(std::move(cimg));
}
